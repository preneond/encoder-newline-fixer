...
def _full_masks(b: int, g: int) -> tuple[torch.Tensor, torch.Tensor]:
    """(all-valid mask, all-zero hard labels) for a [b, g] gap grid."""
    return torch.ones(b, g, dtype=torch.bool), torch.zeros(b, g, dtype=torch.long)


def test_matching_distributions_give_zero_loss() -> None:
    logits = torch.randn(2, 5, 4)
    valid, labels = _full_masks(2, 5)
